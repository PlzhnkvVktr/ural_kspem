package ru.avem.modules.devices.bris.m4122
...
import ru.avem.modules.devices.bris.m4122.M4122Model.Companion.KABS
import ru.avem.modules.devices.bris.m4122.M4122Model.Companion.MEASUREDVOLTAGE
import ru.avem.modules.devices.bris.m4122.M4122Model.Companion.RESISTANCE
import ru.avem.modules.devices.bris.m4122.M4122Model.Companion.RESULT
import ru.avem.modules.devices.bris.m4122.M4122Model.Companion.SETVOLTAGE
import ru.avem.modules.devices.bris.m4122.M4122Model.Companion.STATUS
import ru.avem.modules.devices.cs02021.CS020201Model
...
class M4122(
    ...
    fun getMeasResistance(): List<Int> {
        synchronized(protocolAdapter.connection) {
            ...
            sleep(2)
            val frameSize: Int = maxOf(protocolAdapter.read(inputArray), 0)
    ...
    fun getMeasAbs(): List<Double> {
        synchronized(protocolAdapter.connection) {
            ...
            try {
                println("===============================")
                inpBuf.array().toHexString()
                println("===============================")
                result = inpBuf[0].toInt() // 91 - OK / 103 - NOT OK / 93 - Предыдущий результат из памяти
                val offset = if (result == 91) 1 else 0
                voltage = ByteBuffer.allocate(2).put(inpBuf[4 + offset]).put(inpBuf[3 + offset]).apply { flip() }.short.toInt()
                r1 = ByteBuffer.allocate(4).put(inpBuf[9 + offset]).put(inpBuf[8 + offset]).put(inpBuf[7 + offset]).put(inpBuf[6 + offset]).apply { flip() }.int
                r2 = ByteBuffer.allocate(4).put(inpBuf[14 + offset]).put(inpBuf[13 + offset]).put(inpBuf[12 + offset]).put(inpBuf[11 + offset]).apply { flip() }.int
                abs = ByteBuffer.allocate(2).put(inpBuf[17 + offset]).put(inpBuf[16 + offset]).apply { flip() }.short.toInt()
            } catch (e: Exception) {
                println(e)
            }
    ...
    override fun getRegisterById(idRegister: String) = model.registers[CS020201Model.RESPONDING_PARAM]!!
